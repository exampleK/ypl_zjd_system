...
'</button>'+
  ...
  $.fn.SetEditable = function (options) {
    ...
    $(function(){
        var url="http://127.0.0.1:8000/zjd_main_json"
         //使用getJSON方法取得JSON数据
        $.getJSON(　　
        url,
  ...
  $.fn.UpdateEditable = function (options) {
    var defaults = {
        columnsEd: null,         //Index to editable columns. If null all td editables. Ex.: "1,2,3,4,5"
        $addButton: null,        //Jquery object of "Add" button
        onEdit: function() {},   //Called after edition
    ...
    params = $.extend(defaults, options);
    $(function(){
        // var select_button = $("#select_text").val();
        // alert($("#select_text").val())
        var url="http://127.0.0.1:8000/zjd_worker_json?table_id="+ $("#select_text").val()
         //使用getJSON方法取得JSON数据
        $.getJSON(　　
        url,
        //处理数据 data指向的是返回来的JSON数据
